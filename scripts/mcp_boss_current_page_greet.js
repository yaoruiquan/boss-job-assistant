...
function extractCandidates(text) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const priority = PRIORITY_COMPANY_RE.test(companyLine) ? 100 : 0;
    candidates.push({
...
async function main() {
  ...
  try {
    const initialized = await client.request("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "boss-job-assistant-current-page-greet", version: "0.1.0" }
    });
    ...
    const knownCompanyPass = /字节跳动|杭州今日头条科技|阿里巴巴集团|华为/.test(candidate.companyLine);
    const evidence = {
