...
- `开始自动化`：从当前列表页运行一次受控 MCP 自动化流程，失败会写入 JSONL 日志。
- `详情沟通`：运行一次 `--greet-detail`，适合当前已打开岗位详情页。

插件不直接用 content script 点击 BOSS 页面，也不把主流程改成 DOM `.click()`。content script 只显示一个本页已连接的小浮层。

当前 `开始自动化` 是受控单次流程：从当前列表页筛选一个命中岗位，进入详情，点击 `立即沟通`，再根据强证据判断是否成功。它不是无限循环批量处理；连续 session、去重和翻页会在后续版本接入。
